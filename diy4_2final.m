...
blobBotDist=zeros(2,no_of_blobs+1);
  for iStoreDist=1:no_of_blobs
      ...
      blobBotDist(2,iCall)=sqrt( (( (blobmat(1,1,iStoreDist))-  (mean_bot_position(1))   )^2) + ((   (blobmat(1,2,iStoreDist))-  (mean_bot_position(2))   )^2)    );
...
stats=regionprops(bot_head_labeled,'centroid');
 ...
 %to get the location of bot--end- 
  ...
  h = msgbox(['Angle between blob and bot: ' num2str(botBlobAngle)], 'Show');%display msg box
  if((botBlobAngle<AngleThresh)&&(botBlobAngle>(-AngleThresh)))   %angles are in radian
    flagBotAngleOk=1;
    break
  elseif ((botBlobAngle>=AngleThresh)&&(botBlobAngle<1.0))
      fprintf(arduino,'%c','r');
      pause(0.05);
  elseif ((botBlobAngle<=(-AngleThresh))&&(botBlobAngle>-1.0))
      fprintf(arduino,'%c','l');
      pause(0.05);      
  elseif (botBlobAngle>=1.0)
      fprintf(arduino,'%c','r');
      pause(0.05);
      fprintf(arduino,'%c','r');
      pause(0.05);
...
stats=regionprops(bot_head_labeled,'centroid');
  ...
  bot_angle = -atan2(bot_head_position(2)-bot_body_position(2),bot_head_position(1)-bot_body_position(1));%important variable  
   ...
   %to get the location of bot--end-
   GettheDist=sqrt( (( (blobmat(1,1,iStoreDist))-  (mean_bot_position(1))   )^2) + ((   (blobmat(1,2,iStoreDist))-  (mean_bot_position(2))   )^2)    );
   
  h = msgbox(['Distance between blob and bot: ' num2str(GettheDist)], 'Show');%display msg box 
 if(GettheDist<ForwardThresh)
 flagBotBlobOk=1;
 elseif((GettheDist>=ForwardThresh)&&(GettheDist<1000))
   fprintf(arduino,'%c','f');   
    pause(0.1);
    
 elseif(GettheDist>=1000)
   fprintf(arduino,'%c','f');   
    pause(0.1);
    fprintf(arduino,'%c','f');   
    pause(0.1);
 ...
 if((flagBotAngleOk==1)&&(flagBotBlobOk==1))
   break
 end 
